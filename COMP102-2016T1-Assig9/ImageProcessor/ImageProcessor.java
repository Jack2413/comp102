...
public class ImageProcessor{
        ...
        for(int row=0; row<this.image.length; row++){
            for(int col=0; col<this.image[row].length; col++){
                if(this.image[row][col]>=128){
                    this.image[row][col]=(int)Math.abs((this.image[row][col]-128)*0.2)+this.image[row][col];
                }
                if(this.image[row][col]<128){
                    this.image[row][col]=this.image[row][col]-(int)Math.abs((this.image[row][col]-128)*0.2);
                }
                if(this.image[row][col]<0){
                    this.image[row][col]=0;
                }
    ...
    public void rotateImage180(){
        ...
        for(int row=0; row<this.image.length; row++){
            for(int col=0; col<this.image[0].length; col++){
                image [row][col]=temp [this.image.length-row-1][this.image[0].length-col-1];
    ...
    public void rotateImage90(){
        ...
        for(int row=0; row<this.image[0].length; row++){
            for(int col=0; col<this.image.length; col++){
                image [col][row]=temp [row][image.length-col-1];
    ...
    public void expandImage(){
        ...
        for(int row=0; row<this.image.length-1; row+=2){
            int colt=0;
            for(int col=0; col<this.image[0].length-1; col+=2){

                image [row][col]=temp[rowt][colt];
                image [row+1][col]=temp[rowt][colt];
                image [row][col+1]=temp[rowt][colt];
                image [row+1][col+1]=temp[rowt][colt];
                colt++;
    ...
    public void mergeImage(){
        ...
        for(int row=0; row<rows; row++){
            for(int col=0; col<cols; col++){
                temp [row][col]=(image [row][col]+other[row][col])/2;
    ...
    public void zoomImage(){
        ...
        for(int row=0; row<this.image.length; row++){
            for(int col=0; col<this.image[0].length; col++){
                ...
                int StartR = (int)(this.selectedRow+(distanceR*3.0/4.0));
                int StartC = (int)(this.selectedCol+(distanceC*3.0/4.0));
                if(StartR>=0&&StartC>=0&&StartR<this.image.length&StartC<this.image[0].length){
                    image[row][col]=temp[StartR][StartC];
                }
    ...
    public void redisplayImage(){
        ...
        for(int row=0; row<this.image.length; row++){
            ...
            for(int col=0; col<this.image[0].length; col++){
                ...
                UI.fillRect(270+x, 100+y, this.pixelSize, this.pixelSize);
            }
        }
