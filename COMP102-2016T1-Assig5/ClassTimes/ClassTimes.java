...
public class ClassTimes {
    ...
    public void printCourse(String targetCourse){
        ...
        try {
            ...
            while(scan.hasNext()){
                ...
                if(targetCourse.equals(course)){
                    UI.printf("%s %s %s (%d) (%d) %s %n",course,classtype,day,starttime,endtime,room);
                }   
            }
        }
        catch(IOException e) {UI.printf("File Failure %s \n", e);}
    ...
    public void printRoom(String targetRoom) {
        ...
        try {
            ...
            if(count==0){
                UI.printf("No courses are booked in %s \n",targetRoom);
            }
            else{
                UI.printf("There are %d classes in %s \n",count,targetRoom);
            }
        }
        catch(IOException e) {UI.printf("File Failure %s \n", e);}
        UI.println("=========================");
        ...
        try {
            ...
            while(scan.hasNext()){
                ...
                if((room.equals(targetRoom1)||(room.equals(targetRoom2)))&&(day.equals(targetDay))){
                    UI.printf("%s %s %s (%d) (%d) %n",course,classtype,day,starttime,endtime);
                }   
            }
        }
        catch(IOException e) {UI.printf("File Failure %s \n", e);}
        ...
        try {
            ...
            while(scan.hasNext()){
                ...
                if(targetRoom.equals(room)){ 
                    ...
                    UI.printf("%s %s %s (%d) (%d) %s (%d) %n",course,classtype,day,starttime,endtime,room,sum);
                    count++;
                    mean=sum/count;
                }   
            }
    ...
    public void potentialDisruptions(String building, String targetDay, int targetStart, int targetEnd){
        ...
        try {
            ...
            while(scan.hasNext()){
                ...
                int endtime = scan.nextInt();
                String room = scan.next();
                if(room.startsWith(building)&&(day.equals(targetDay))&&(starttime==targetStart)&&(endtime==targetEnd)){
                    UI.printf("%s %s %s (%d) (%d) %s %n",course,classtype,day,starttime,endtime,room);
                }   
            }
        }
