...
public class DominoGame{
    ...
    private int releasedpos=0;
    //  selected position in the hand.
    private int numTT=0;
    private int numTB=0;
    private int numHT=0;
    private int numHB=0;
    ...
    public DominoGame(){
        ...
        UI.addButton("Resart", this::restart);
    ...
    public void drawHand(){
        ...
        UI.drawRect(HAND_LEFT-2,HAND_TOP-2,NUM_HAND*(DOMINO_SPACING)+2,DOMINO_HEIGHT+4);

        if(this.hand[selectedPos]!=null){
            UI.setLineWidth(1);
            UI.setColor(Color.green);
            UI.drawRect(HAND_LEFT-1+selectedPos*DOMINO_SPACING,HAND_TOP-1,(DOMINO_SPACING)-3,DOMINO_HEIGHT+1);
        }
    ...
    public void placeDomino(){
        Domino pick = this.hand[selectedPos];
        if(this.hand[selectedPos]!=null&&this.table.size()>0){

            this. numHT=this.hand[selectedPos].getTop();
            this. numHB=this.hand[selectedPos].getBottom();
            this. numTT=table.get(table.size()-1).getTop();
            this. numTB=table.get(table.size()-1).getBottom();
            if (numHT==numTT||numHB==numTB){
    ...
    public void drawTable(){
        double table_top=this.TABLE_TOP;
        double cou=0;
        ...
        for (int i=0; i<this.table.size(); i++){
            if (this.table.get(i)!=null){
                ...
                this.table.get(i).draw(TABLE_LEFT + cou*(DOMINO_SPACING+1), table_top+row*(DOMINO_HEIGHT+10));
                cou++;
    ...
    public void suggestDomino(){
        ...
        for (int i=0; i<this.hand.length; i++){
            if(this.hand[i]!=null&&this.table.size()>0){
                this. numHT=this.hand[i].getTop();
                this. numHB=this.hand[i].getBottom();
                this. numTT=table.get(table.size()-1).getTop();
                this. numTB=table.get(table.size()-1).getBottom();

                if (numTT==numHT||numTT==numHB||numTB==numHT||numTB==numHB){
                    UI.printMessage("Suggesting ["+numHT+"-"+numHB+"] at index "+ i+ " to be placed on position " +table.size());
                    run = false;
    ...
    public void doMouse(String action, double x, double y){
        if(action.equals("pressed")){
            ...
            x >= HAND_LEFT && x <= HAND_LEFT + NUM_HAND*DOMINO_SPACING) {
                ...
                this.redraw();
                UI.clearText();
                UI.println("selectedPos "+this.selectedPos);
            }
        }
            ...
            x >= HAND_LEFT && x <= HAND_LEFT + NUM_HAND*DOMINO_SPACING) {
                ...
                if(releasedpos>pressedpos){
                    for (int i= 0 ;i<(releasedpos-pressedpos); i++){
                        Domino temp =hand[selectedPos];
                        hand[selectedPos]=hand[selectedPos+1];
                        hand[selectedPos+1]= temp;
                        selectedPos++;
                ...
                else if(pressedpos>releasedpos){
